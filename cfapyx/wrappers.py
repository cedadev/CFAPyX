...
class FragmentArrayWrapper(ArrayLike):
    ...
    def _optimise_chunks(self):
        ...
        nchunks = normalize_partition_chunks(
            auto_chunks, self.shape, self.dtype, self.named_dims
        )
    ...
    def _create_partitions(self, fragments):
        ...
        for coord in partition_coords:
            ...
            for dim, c in enumerate(coord):
                ...
                ext = slice(partition_cumul[dim][c], partition_cumul[dim][c + 1])
                internal.append(ext)
    ...
    def _assemble_array(self, dsk, array_name, dask_chunks):
        ...
        darr = da.Array(
            dsk, array_name, chunks=dask_chunks, dtype=self.dtype, meta=meta
        )
        return darr
